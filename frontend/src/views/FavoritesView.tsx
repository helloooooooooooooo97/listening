...
const TYPE_META = {
  audio: { icon: HiMusicalNote, gradient: 'var(--card-gradient)', label: '音频', empty: '还没有收藏音频' },
  clip: { icon: HiBookmark, gradient: 'var(--clip-gradient)', label: '片段', empty: '还没有收藏片段' },
  word: { icon: HiTag, gradient: 'var(--ielts-gradient)', label: '单词', empty: '还没有收藏单词' },
} as const;
        ...
        {!loaded ? (
          ...
          <div className="space-y-3">
            {activeItems.map(item => {
              ...
              return (
                <div key={item.id}
                  className="group rounded-xl p-4 transition-all duration-200 hover:bg-[var(--bg-tertiary)] cursor-pointer"
                    ...
                    <div className="flex-1 min-w-0">
                      {item.item_type === 'clip' ? (
                        ...
                        <>
                          ...
                          <p className="text-xs text-tertiary mt-0.5">{item.subtitle || '单词'}</p>
                        </>
                      ) : (
                        <>
                          <p className="text-sm font-medium text-primary">{item.title}</p>
                          <p className="text-xs text-tertiary mt-0.5">{item.subtitle}</p>
                          <p className="text-xs text-tertiary mt-0.5">{item.created_at?.slice(0, 10)}</p>
                        </>
                      )}
                    </div>
                    {item.item_type !== 'word' && (
