  ...
  return (
    <div className="h-full flex flex-col bg-[var(--bg-primary)] overflow-hidden">
      <div className="flex-shrink-0 px-8 pt-10 pb-4">
        <h1 className="text-2xl font-extrabold text-primary tracking-tight">听写记录</h1>
      </div>
      <div className="flex-1 overflow-y-auto px-8 py-6">
        {groups.length === 0 ? (
          <div className="text-center py-16">
            <HiPencilSquare size={40} className="text-tertiary mx-auto mb-4" />
            <p className="text-tertiary text-sm">还没有听写记录</p>
            <p className="text-tertiary text-xs mt-1">在音频中进入听写模式开始练习</p>
          </div>
        ) : (
          <div className="space-y-6 max-w-3xl mx-auto">
            {groups.map((g) => {
              ...
              return (
                <div key={g.audio_id} className="rounded-2xl overflow-hidden" style={{ background: 'var(--bg-tertiary)' }}>
                  {/* Audio header */}
                  <div
                    onClick={() => toggleExpand(g.audio_id)}
                    className="flex items-center gap-4 px-6 py-4 cursor-pointer hover:bg-[var(--bg-tertiary)] transition-colors"
                  >
                    {/* Score circle */}
                    <div
                    ...
                    <button
                      ...
                      className="w-9 h-9 rounded-full flex items-center justify-center bg-[var(--bg-tertiary)] hover:bg-[var(--bg-active)] text-tertiary hover:text-secondary transition-colors cursor-pointer flex-shrink-0"
                      title="播放此音频"
                    >
                      <HiPlay size={14} />
                    </button>
                  ...
                  {isExpanded && (
                    <div className="px-6 pb-5 space-y-3 border-t border-[var(--border-secondary)] pt-4">
                      {g.records.map((r, i) => {
                        ...
                        return (
                          <div
                            key={i}
                            className="flex gap-4 py-3 border-b border-[var(--border-secondary)] last:border-0"
                          >
                            {/* Left: score + index */}
                            ...
                            <div className="flex-1 min-w-0 space-y-1.5">
                              ...
                              {r.expected_text && (
                                <div className="flex items-start gap-2">
                                  <span className="text-xs text-emerald-400/70 flex-shrink-0 mt-0.5">正确</span>
                                  <p className="text-[14px] text-emerald-400/70 leading-relaxed break-all">
                                    {r.expected_text}
                                  </p>
                                </div>
                              )}
                              ...
                              {hasDetail && !isPerfect && r.user_input && (
                                <div className="flex items-start gap-2">
                                  <span className="text-xs text-red-500/70 flex-shrink-0 mt-0.5">你的</span>
                                  <p className="text-[14px] text-red-300/50 leading-relaxed break-all line-through">
                                    {r.user_input || <span className="italic text-tertiary">(未输入)</span>}
                                  </p>
                                </div>
                              )}
                              {r.score === 0 && !r.user_input && (
                                <div className="flex items-start gap-2">
                                  <span className="text-xs text-tertiary flex-shrink-0 mt-0.5">跳过</span>
                                  <p className="text-xs text-tertiary italic">已跳过此句</p>
                                </div>
                              )}
                              {/* Date */}
                            ...
                            <button
                              ...
                              className="w-8 h-8 rounded-full flex items-center justify-center bg-[var(--bg-tertiary)] hover:bg-[var(--bg-active)] text-tertiary hover:text-secondary transition-colors cursor-pointer flex-shrink-0 self-center"
                              title="播放此句"
                            >
                              <HiPlay size={12} />
                            </button>
