...
export default function DictationTrend({ trend, loading }: { trend: DictationScore[]; loading: boolean }) {
  if (loading) {
    return (
      <div>
        <h2 className="text-lg font-bold text-primary mb-4">听写趋势</h2>
          ...
          {trend.map((s, i) => {
            ...
            return (
              <div key={i} className="flex-1 flex flex-col items-center gap-1 group h-full justify-end">
                <span className="text-xs text-white/0 group-hover:text-secondary font-mono transition-opacity">{s.score}%</span>
                ...
                <span className="text-[8px] text-tertiary truncate w-full text-center">{s.date.slice(5)}</span>
              </div>
            );
          })}
        </div>
