      ...
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          <h1 className="text-3xl font-extrabold text-primary mb-2 flex items-center justify-center gap-2">
            <HiSparkles size={24} style={{ color: 'var(--accent)' }} /> 单词消除
          </h1>
          <p className="text-sm text-tertiary">消除 3 个相同单词 = 复习成功</p>
        </div>

        {/* Word source */}
        <div className="mb-6">
          <p className="text-xs text-tertiary font-medium mb-2 uppercase tracking-wider">单词来源</p>
          <div className="flex gap-2">
            {sources.map(s => (
              <button key={s.key} onClick={() => setSource(s.key)}
                className={`flex items-center gap-1.5 flex-1 py-2 rounded-lg text-xs font-medium transition-colors cursor-pointer ${
                  source === s.key
        ...
        {/* Difficulty */}
        <div className="mb-8">
          <p className="text-xs text-tertiary font-medium mb-2 uppercase tracking-wider">难度</p>
          <div className="space-y-2">
            {difficulties.map(d => (
              <button key={d.key} onClick={() => setDifficulty(d.key)}
                ...
                <p className="text-sm font-semibold flex items-center gap-2">{DIFF_ICONS[d.key]} {d.label}</p>
        ...
        <div className="flex gap-3">
          <button onClick={onBack}
            className="flex-1 py-3 rounded-xl text-sm font-medium text-tertiary hover:text-secondary hover:bg-[var(--bg-hover)] transition-colors cursor-pointer">
            返回
          </button>
          <button onClick={() => onStart(difficulty, source)}
            className="flex-1 py-3 rounded-xl text-sm font-semibold bg-[var(--accent)] on-accent hover:opacity-90 transition-opacity cursor-pointer flex items-center justify-center gap-1.5">
            <HiSparkles size={14} /> 开始游戏
          </button>
        </div>
      </div>
    </div>
